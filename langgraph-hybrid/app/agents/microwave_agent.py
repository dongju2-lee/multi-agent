...
# 로거 설정
logger = setup_logger("microwave_agent")
...
async def init_mcp_client():
    ...
    if _mcp_client is None:
        ...
        mcp_config = {
            "microwave": {
                "url": os.environ.get("MICROWAVE_MCP_URL", "http://0.0.0.0:8005/sse"),
                "transport": "sse",
            },
        }
...
async def get_microwave_agent_async():
    """전자레인지 제어 에이전트의 싱글톤 인스턴스를 비동기적으로 생성합니다."""
    ...
    if _agent_instance is None:
        ...
        try:
            ...
            ("system", """당신은 전자레인지를 제어하는 스마트홈 에이전트입니다. 
전자레인지의 전원 제어, 모드 변경, 타이머 설정 및 요리 시작 기능을 제어할 수 있습니다.
다양한 모드(전자레인지, 베이킹, 그릴링, 오븐)를 지원합니다.

응답은 항상 한국어로 제공하세요. 제공된 MCP 도구를 사용하여 전자레인지를 제어하세요."""),
            ...
            logger.info("전자레인지 제어 에이전트 초기화 완료")
        except Exception as e:
            logger.error(f"전자레인지 제어 에이전트 초기화 중 오류 발생: {str(e)}")
            raise
        
    return _agent_instance


def get_microwave_agent():
    """전자레인지 제어 에이전트의 싱글톤 인스턴스를 반환합니다."""
    global _agent_instance
    if _agent_instance is None:
        try:
            # 비동기 초기화 함수를 동기적으로 실행
            logger.info("전자레인지 에이전트 비동기 초기화 시작")
            loop = asyncio.get_event_loop()
            _agent_instance = loop.run_until_complete(get_microwave_agent_async())
            logger.info("전자레인지 에이전트 비동기 초기화 완료")
        except Exception as e:
            logger.error(f"전자레인지 에이전트 초기화 실패: {str(e)}")
            raise
...
def microwave_node(state: MessagesState) -> Command[Literal["supervisor"]]:
    ...
    try:
        # 에이전트 인스턴스 가져오기
        logger.info("전자레인지 제어 에이전트 노드 함수 실행 시작")
        microwave_agent = get_microwave_agent()
        ...
        if "messages" in result and result["messages"]:
            ...
            microwave_message = HumanMessage(content=last_message.content, name="microwave_agent")
            logger.info(f"전자레인지 에이전트 응답: '{last_message.content[:1000]}...'")
        else:
            logger.warning("전자레인지 에이전트가 응답을 생성하지 않음")
            microwave_message = HumanMessage(content="응답을 생성할 수 없습니다.", name="microwave_agent")
        
        logger.info("전자레인지 제어 에이전트 작업 완료, 슈퍼바이저로 반환")
        
        # 슈퍼바이저로 돌아가기
        return Command(
            update={"messages": [microwave_message]},
            goto="supervisor"
        )
    except Exception as e:
        logger.error(f"전자레인지 노드 함수 실행 중 오류 발생: {str(e)}")
        error_message = HumanMessage(
            content=f"전자레인지 에이전트 실행 중 오류가 발생했습니다: {str(e)}",
            name="microwave_agent"
        )
        return Command(
