...
def get_routine_tools_with_details() -> List:
    """루틴 관리 도구를 가져오고 상세 정보를 로깅합니다."""
    logger.info("루틴 관리 도구 로딩 시작")
    
    try:
        tools = [register_routine, list_routines, delete_routine, suggest_routine]
        
        # 도구 정보 로깅
        logger.info(f"총 {len(tools)}개의 루틴 관리 도구를 가져왔습니다")
        ...
        logger.info("루틴 관리 도구 로딩 완료")
        return tools
    except Exception as e:
        logger.error(f"루틴 관리 도구 로딩 중 오류 발생: {str(e)}")
        raise
...
def get_routine_agent():
    ...
    if _agent_instance is None:
        try:
            ...
            # 루틴 관리 도구 준비
            tools = get_routine_tools_with_details()
